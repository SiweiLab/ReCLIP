...
RUN_ROOT="${RUN_ROOT:-${ROOT_DIR}/scripts/ablation/runs/topk}"
K_VALUES="${K_VALUES:-3 5 7 10}"
...
run_mutation() {
  local k="$1"
  local out_dir="${RUN_ROOT}/mutation/k${k}"
  mkdir -p "${out_dir}"
  ...
  local cmd=(
    ...
    --top-k "${k}"
    --device "${DEVICE}"
    --output-prefix "mutation_reclip_L32_k${k}"
...
run_ptm() {
  local k="$1"
  local out_dir="${RUN_ROOT}/ptm/k${k}"
  mkdir -p "${out_dir}"
