...
parser.add_argument('--preset', default='auto', choices=['auto', 'classI', 'classII', 'mixed', 'none'],
                    help='Hyperparameter preset. auto resolves from dataset path/name using the SWING README.')
#Language Parameters:
...
parser.add_argument('--k',default=None,type=int,help='size of words (kmer) for the window encoding to be fragmented into')
...
#Doc2Vec Parameters
parser.add_argument('--w',default=None,type=int,help='how large the surrounding window is')
parser.add_argument('--dm',default=None,type=int,help='Doc2Vec dm parameter')
parser.add_argument('--dim',default=None,type=int,help='Doc2Vec dim parameter')
parser.add_argument('--epochs',default=None,type=int,help='Doc2Vec number epochs')
parser.add_argument('--min_count',default=None,type=int,help='Doc2Vec minimum count')
parser.add_argument('--alpha',default=None,type=float,help='Doc2Vec alpha value')
...
#XGBoost Parameters
parser.add_argument('--n_estimators',default=None,type=int,help='XGBoost n estimators')
parser.add_argument('--max_depth',default=None,type=int,help='XGBoost maximum depth')
parser.add_argument('--learning_rate',default=None,type=float,help='XGBoost learning rate')
parser.add_argument('--xgb_device', default='cuda', choices=['auto', 'cuda', 'cpu'],
                    help='XGBoost device. auto matches the official SWING constructor; cuda forces gpu_hist.')
parser.add_argument('--xgb_n_jobs', default=None, type=int, help='Optional XGBoost CPU threads.')
...
PRESET_PARAMS = {
    ...
    'classII': {
        ...
        'w': 12,
        'min_count': 1,
        'alpha': 0.03887032752085429,
        'epochs': 13,
        'n_estimators': 341,
        'max_depth': 9,
        'learning_rate': 0.6534638199102993,
    },
    'mixed': {
        'k': 7,
        'dim': 74,
        ...
        'w': 12,
        'min_count': 1,
        'alpha': 0.03783042872771851,
        'epochs': 10,
        'n_estimators': 269,
        'max_depth': 9,
        'learning_rate': 0.6082359422582875,
    },
}


def resolve_preset_name(data_set, preset):
    if preset != 'auto':
        return preset

    ds = os.path.basename(str(data_set))
    if ds.startswith('ClassI_'):
        return 'classI'
    if ds.startswith('ClassII_'):
        return 'classII'
    if ds.startswith('MixedClass_'):
        return 'mixed'
    return 'none'


def apply_preset_defaults(parsed_args):
    preset_name = resolve_preset_name(parsed_args.data_set, parsed_args.preset)
    if preset_name == 'none':
        return preset_name

    preset_params = PRESET_PARAMS[preset_name]
...
resolved_preset = apply_preset_defaults(args)

missing_required = [
    key for key in ['k', 'w', 'dm', 'dim', 'epochs', 'min_count', 'alpha', 'n_estimators', 'max_depth', 'learning_rate']
    if getattr(args, key) is None
]
if missing_required:
    raise ValueError(
        f"Missing SWING hyperparameters: {missing_required}. "
        "Use --preset auto/classI/classII/mixed or pass them explicitly."
    )
...
print('Using preset:', resolved_preset)
print(
    'Resolved params:',
    {
        'k': args.k,
...
if classifier == 'XGBoost':
    ...
    if args.xgb_n_jobs is not None:
        xgb_base['n_jobs'] = args.xgb_n_jobs

    # Match the official SWING constructor unless the user explicitly forces CUDA.
    if args.xgb_device == 'cuda':
        classifier_model = XGBClassifier(**xgb_base, tree_method='gpu_hist', predictor='gpu_predictor')
        _xgb_cuda_requested = True
    elif args.xgb_device == 'cpu':
        classifier_model = XGBClassifier(**xgb_base, tree_method='hist')
        _xgb_cuda_requested = False
    else:
        classifier_model = XGBClassifier(**xgb_base)
        _xgb_cuda_requested = False
...
try:
    ...
    if classifier == 'XGBoost' and '_xgb_cuda_requested' in globals() and _xgb_cuda_requested:
        print(f'[warn] XGBoost CUDA failed, fallback to CPU. reason: {e}')
        ...
        if args.xgb_n_jobs is not None:
            classifier_model.set_params(n_jobs=args.xgb_n_jobs)
        classifier_model.fit(tr_vecs, tr_y)
    else:
        raise
