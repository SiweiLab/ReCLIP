...
def main():
    ...
    def embed(example):
        ...
        hidden_states = output[0]                      # (1, L, hidden)
        embedded = torch.mean(hidden_states, dim=1).squeeze()  # (hidden,)
        return embedded.detach().cpu().numpy()

    embeddings = list(map(embed, tqdm(data)))
    embedding_array = pd.DataFrame(np.vstack(embeddings))
    embedding_array.to_csv('swing_bert_baseline_mutint_scv/embedding.csv')
